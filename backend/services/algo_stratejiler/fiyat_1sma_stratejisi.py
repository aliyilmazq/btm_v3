...
def run_strategy(data, params=None):
    try:
        ...
        # Sharpe ve Sortino Oranı (günlük portföy getirisiyle)
        if len(portfoy_degeri) > 1:
            gunluk_getiriler = np.diff(portfoy_degeri) / portfoy_degeri[:-1]
            ort_gunluk = np.mean(gunluk_getiriler)
            std_gunluk = np.std(gunluk_getiriler)
            std_negatif = np.std(gunluk_getiriler[gunluk_getiriler < 0]) if np.any(gunluk_getiriler < 0) else 0.0
            sharpe = (ort_gunluk / (std_gunluk + 1e-9)) * np.sqrt(252) if std_gunluk > 0 else 0
            sortino = (ort_gunluk / (std_negatif + 1e-9)) * np.sqrt(252) if std_negatif > 0 else 0
        else:
            sharpe = 0
            sortino = 0
